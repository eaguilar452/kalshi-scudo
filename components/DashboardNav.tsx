        ...
        <div className="flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-bg-card border border-slate-border flex items-center justify-center">
              <div className="light-green" style={{ width: "8px", height: "8px" }} />
            </div>
              ...
              return (
                <a
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    isActive
                      ? "text-green-glow bg-green-subtle"
                      : "text-slate-text hover:text-white hover:bg-bg-hover"
                  }`}
                >
                  <span className={`text-xs ${isActive ? "text-green-glow" : "text-slate-muted"}`}>
                    {item.icon}
                  </span>
                  {item.label}
                </a>
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 text-sm text-slate-text hover:text-white transition-colors"
            >
            ...
            {menuOpen && (
              <div className="absolute right-0 mt-2 w-48 gl-card p-1 shadow-xl z-50">
                <button
                  ...
                  className="w-full text-left px-4 py-2.5 text-sm text-slate-text hover:text-red-glow hover:bg-red-subtle rounded-lg transition-colors"
                >
                  Sign Out
                </button>
              </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <a
              key={item.href}
              href={item.href}
              className={`flex-1 text-center py-3 text-xs font-medium transition-all ${
                isActive
                  ? "text-green-glow border-b-2 border-green-glow"
                  : "text-slate-muted"
              }`}
            >
              {item.label}
